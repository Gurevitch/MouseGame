...
> **Architecture:** See [ARCHITECTURE.md](ARCHITECTURE.md) for refactor details.
> **Issues:** See [FIXME.md](FIXME.md) for known bugs and deferred items.

---

## Refactor progress (2026-04-18)

| Phase | Status | Notes |
|-------|--------|-------|
| 1. Scaffolding (docs/, event_bus stub) | DONE | |
| 2. Atlas pipeline (pack_atlas.py + atlas.go) | DONE | Flood-fill color-key from edge, baseline-align, per-cell keying. 6 character atlases. |
| 3. JSON scene loader | DONE | All 13 scenes load from `assets/data/scenes/*.json`. scene.go 1511 → 864 lines. |
| 4. NPC atlas migration | DONE | 5 kids use `applyKidAtlas`; deleted 4 legacy loader variants. |
| 4b. NPC state machine + rules evaluator | DEFERRED | Needed before setupCampCallbacks closures can be dismantled. |
| 5. Sequence player infrastructure | DONE | JSON seq loader + new step types (`npc_anim`, `npc_strange`, `scene_bg`, `player_sleep`, `player_wake`, `hide_player`, `start_day`). `night_bedtime.json` authored. |
| 5b. Sequence swap-in (replace nightSceneUpdate) | DEFERRED | Needs live playtest to verify parity. |
| 6. Game god-object collapse | PARTIAL | Moved travelMap display state, deleted dead mapReveal, consolidated alt-BG caches. Story flags + night fields deferred — see FIXME "Refactor: Phase 6 deferred". |
| 7. Subpackage split | DEFERRED | Blocked on Phase 6 — noted in FIXME. |
| 8. Docs + verification | IN PROGRESS | |
